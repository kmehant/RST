...
import guidance
from guidance._program import *
from guidance import _grammar



probs = {}   # dictionary to store logits for every generated token
generated_tokens = []  # stored every generated token used later in probs dictionary
tokenizer = None  # used to access tokenizer locally
...
class Transformers(LLM):
    ...
    def encode_(self, string):
 
        out_temp = self._tokenizer(string)["input_ids"]
        out = []
        global vocab
...
class TransformersSession(LLMSession):
    ...
    async def __call__(self, prompt, stop=None, stop_regex=None, temperature=None, n=1, max_tokens=1000, logprobs=None, top_p=1.0, echo=False, logit_bias=None, token_healing=None, pattern=None, stream=False, cache_seed=0, caching=None, last_token=None):
        ...
        if not in_cache or not_caching:
            ...
            if prefix_match_len < len(self._prefix_cache): # prefix_match_len > 0 and 
                # self._past_key_values = [key_value[:,:prefix_match_len,:] for key_value in self._past_key_values]  #for starcoder
                # self._past_key_values = tuple(tuple(key_value[:,:,:prefix_match_len,:] for key_value in key_values) for key_values in self._past_key_values) # for codellama
            ...
            #processors.append(dummyLogitProcessor(self.llm.decode, self.llm.encode_))
            # processors.append(NoBadWordsLogitsProcessor_custom(bad_tokens, 50256))
            # processors.append(ReturnLogitsProcessor(self.llm.decode))

            if stop_regex is not None:
                stoppers.append(RegexStoppingCriteria(stop_regex, self.llm.decode, len(coded_prompt)))
            stoppers.append(IndentationStoppingCriteria(self.llm.decode))
...
class ReturnLogitsProcessor():
    ...
    def __call__(self, input_ids, scores):
        ...
        global probs
        max_ind = np.argmax(scores.detach().cpu().numpy())
        token = self.decode([max_ind])
        generated_tokens.append(input_ids[0])
        if token not in probs:
            probs[token] = []
            probs[token].append(scores.detach().cpu().numpy())
        else:
            probs[token].append(scores.detach().cpu().numpy())
#         print(probs)
#         print(scores)
...
class IndentationStoppingCriteria():
    def __init__(self, decode):
        self.decode = decode
...
#         max_ind = np.argmax(scores.detach().cpu().numpy())
        complete_str = self.decode(input_ids[0])
        last_token = self.decode([input_ids[0][-1]])
        module_spaces = cnt_spaces_left(complete_str.split("\n")[1])
        cur_line = complete_str.split("\n")[-1]
        ...
        if complete_str.rstrip(' ')[-1] == "\n":
            if cnt_spaces_left(complete_str.split("\n")[-1]) > module_spaces:
                cnt_double_space = [line for line in complete_str.split("\n") if cnt_spaces_left(line)==module_spaces]
                if len(cnt_double_space) == 1:
                    return True
            # if (cnt_spaces_left(complete_str.split("\n")[-1]) - module_spaces)==1 and len(total_keys) == len([m.strip() for m in guidance.library._geneach.select_generated_id[1]]):
            #     return True
            
        # if "\n" in last_token:
        #     if regex.match('^\n\s+',last_token) or regex.match('^#\s+',last_token):
                
        #         if regex.match('^#\s+',last_token):
        #             last_token = last_token.replace("#", " ")
                    
        #         lines = complete_str.split("\n")
        #         spaces_prev_line = ""
        #         for char in lines[-2]:
        #             if char == " ":
        #                 spaces_prev_line += char
        #             else:
        #                 break
        #         last_token = last_token.lstrip('\n')
        #         if(last_token <= spaces_prev_line) and cnt_spaces_right(last_token) <= (cnt_spaces_left(complete_str.split("\n")[1]) + 2):
        #             return True
            # else:
            #     return True
