...
cur_iteration = None   # iterator variable to indicate current iteration going on. Can be used to put required function in select
iterator = None   #iterator variable to indicate geneach is in progress
select_generated_id = {}   #dictionary to maintain uniqueness in select in geneach block
# select_generated_id[1] = []

async def geneach(list_name, stop=None, max_iterations=100, min_iterations=0, num_iterations=None, hidden=False, join="", single_call=False, single_call_temperature=0.0, single_call_max_tokens=500, single_call_top_p=1.0, _parser_context=None, unique=None):
    ...
    global select_generated_id
    global iterator

    iterator = True

    if len(select_generated_id)==0 and unique:   #len=0 condition to avoid reset in dynamic template change
        for id in unique:
            select_generated_id[id] = []
    ...
    iterator = None
    select_generated = {}   #reset iterator and select uniquness
